...
export default function SubcategoriaPage({ 
  ...
  // Filter events by category and subcategory
  const subcategoryEvents = Object.values(eventData).filter(
    event => event.category === params.categoria && event.subcategory === params.subcategoria
  );

  return (
    <div className="container mx-auto py-8 px-4">
      <h1 className="text-3xl font-bold mb-8 capitalize text-foreground">
        {params.categoria} - {params.subcategoria.replace('-', ' ')}
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-3">
          {subcategoryEvents.length === 0 ? (
            <p className="text-foreground/70">Nenhum evento encontrado nesta subcategoria.</p>
          ) : (
